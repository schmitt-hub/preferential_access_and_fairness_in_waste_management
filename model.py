...
def build_model(users_and_facs_df, travel_dict, users, facs, budget_factor=1.0, strict_assign_to_one=False,
    ...
    # the facilities that have a users from which the travel willingness is higher than cutoff
    ...
    for i in m.users:
        if i not in m.assignable_users:
            ...
            if not assignment_found:
                results["solution_details"]["assignment"][i] = None
                unassigned_ctr += 1
    results["solution_details"]["areas_needing_postprocessing"] = unassigned_ctr
    return True, results


def build_postprocessing_model(model, results):
    """
    build the post processing model
    :param model: the main model
    :param results: dictionary of the results of the main optimization step
    :return: the built post processing model
    """
    print('Build postprocessing model...')
    start = time()
    m = pyo.ConcreteModel()
    m.assignment = results['solution_details']["assignment"]
    ...
    m.assignable_facs = pyo.Set(initialize=results['solution_details']["open_facs"])
    ...
    m.cap = pyo.Param(m.facs, initialize=cap_init)
    m.exp_travelers = pyo.Param(m.travel_pairs, initialize=exp_travelers_init)
    m.strict_assign_to_one = pyo.Param(initialize=False)
    m.x = pyo.Var(m.travel_pairs, within=pyo.Binary)
...
def optimize_postprocessing_model(m, results, threads=1, tolerance=0.0, print_sol=False, log_file=None,
                                  preqlinearize=-1):
    """
    ...
    :param results: dictionary of results of the main optimization step
    :param threads: number of threads
    :param tolerance: tolerance on the optimality of the solution
    :param print_sol: boolean indicating whether the solution should be printed
    :param log_file: name of the log file. If "None", no log file will be produced
...
def solve_model_naively(users_and_facs_df, travel_dict, users, facs, budget_factor=1.0, strict_assign_to_one=False,
                        cap_factor=1.5, cutoff=0.2, main_threads=1, main_tolerance=5e-3, main_time_limit=20000,
    ...
    :param main_log_file: name of the log file for the main optimization step;
                          if "None", no log file will be produced
    ...
    if not is_feasible:
        ...
        return is_feasible, {}
    if results['solution_details']['areas_needing_postprocessing'] > 0:
        print()
