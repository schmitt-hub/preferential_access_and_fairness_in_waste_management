...
def greedy_assign_users(users_and_facs_df, travel_dict, users, facs, open_facs, exp_travelers_dict, cap_dict):
    """
...
def greedy(users_and_facs_df, travel_dict, users, facs, lb=0.0, budget_factor=1.0, cap_factor=1.5, turnover_factor=0.02,
           tolerance=5e-3, time_limit=20000, iteration_limit=20):
    """
    greedy heuristic
    :param users_and_facs_df: dataframe of the user and facility related input data
    :param travel_dict: dictionary of the travel probabilities from each user to each facility
    ...
    sorted_zip_df = users_and_facs_df.iloc[facs].sort_values(by=['capacity'], ascending=False)
    ...
    best_open_facs = sorted_zip_df.index[:budget]
    ...
    for i in range(nr_of_users):
        for j in range(nr_of_facs):
            travel_matrix[i][j] = travel_dict[i][facs[j]]
    exp_travelers_matrix = np.array(users_and_facs_df.loc[users]['population']).reshape(-1, 1) * travel_matrix
    exp_travelers_dict = {i: {facs[j]: exp_travelers_matrix[i][j] for j in range(nr_of_facs)}
                          for i in range(nr_of_users)}

    while time()-start_time <= time_limit:
        # assign users
        is_feasible_assignment, assignment_results = greedy_assign_users(users_and_facs_df, travel_dict,  users, facs,
